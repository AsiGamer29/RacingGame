...
public:
    Kart(ModulePhysics* physics, int _x, int _y, Module* _listener, Texture2D _texture, int _engineSound, int _boostSound, Application* _app)
        ...
        , boostSound(_boostSound) // A�adir el sonido de boost
        , isMoving(false)
        , isBoosting(false) // A�adir estado de boost
    ...
    void Update() override
    {
        ...
        // Reproduce el sonido del motor si el kart se est� moviendo
    ...
    void HandleInput()
    {
        ...
        // Si se presiona Shift, aumentar la velocidad m�xima y reproducir el sonido de boost
        ...
        // Deceleraci�n adicional cuando se deja de pulsar Shift
...
private:
    ...
    float speed;
    float rotation;
    const float deceleration = 0.05f; // Valor de deceleraci�n
    int engineSound;
    int boostSound; // A�adir variable para el sonido de boost
    bool isMoving;
    bool isBoosting; // A�adir estado de boost
